...
describe('useNodeStatusStream', () => {
  ...
  it('normalizes townhouse-{type} container names and ignores other namespaces', async () => {
    const { result } = renderHook(() =>
      useNodeStatusStream({ url: 'ws://test/api/metrics' })
    );
    ...
    await act(async () => {
      MockWebSocket.instances[0]!.acceptOpen();
      // Production DockerOrchestrator names: townhouse-{type}
      MockWebSocket.instances[0]!.pushMessage({
        type: 'nodeState',
        payload: { name: 'townhouse-town', state: 'running' },
        ts: Date.now(),
      });
      MockWebSocket.instances[0]!.pushMessage({
        type: 'nodeState',
        payload: { name: 'townhouse-mill', state: 'paused' },
        ts: Date.now(),
      });
