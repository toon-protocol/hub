 ...
 * (`scripts/hub-test-infra.sh up` warms images; operator runs
 * `hub init --preset=demo` + `hub up`). Ditto build artifact
 ...
 *   1. bash scripts/hub-test-infra.sh up
 *   2. hub init --preset=demo --config-dir <dir> --yes
 *      (preset locks: 1 town + 1 mill + 1 dvm, fees=0, transport=ator;
 *      see packages/hub/src/presets/demo.ts)
 *   3. hub up --config-dir <dir>
 ...
 *   to the Hub Fastify API on http://127.0.0.1:9400.
 *
 * AC-D10-6 note on infra script: hub-test-infra.sh does NOT need
 ...
 * `hub init --preset=demo` time — downstream of this script. The
...
// Mirrors AggregatedEarnings from packages/hub/src/earnings/aggregator.ts.
...
test.describe('@real-stack Demo composition (D10)', () => {
  test.skip(
    !REAL_STACK,
    'Real-stack E2E disabled — set TOWNHOUSE_E2E_REAL_STACK=1 (and bring up the demo stack: hub init --preset=demo + hub up)'
  );
