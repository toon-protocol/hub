...
describe('Home view', () => {
  ...
  it('renders error state with retry when /api/nodes fails', async () => {
    ...
    expect(screen.getByText(/could not reach hub api/i)).toBeInTheDocument();
  ...
  it('overrides node state from WS nodeState messages (AC-3, AC-10)', async () => {
    ...
    // Use the realistic `hub-town` name that DockerOrchestrator emits in
    // production (CONTAINER_PREFIX `hub-` + node type) — the hook strips
    ...
    await act(async () => {
      ...
      MockWebSocket.instances[0]!.pushMessage({
        type: 'nodeState',
        payload: { name: 'hub-town', state: 'paused' },
        ts: Date.now(),
      });
    });
