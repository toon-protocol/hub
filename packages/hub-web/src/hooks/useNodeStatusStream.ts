...
export interface UseNodeStatusStreamResult {
   ...
   * normalized from the WS server's `hub-{type}` container names, and
   * non-node names (`pull:*`, `connector`, `hub-connector`, …) are
   * filtered out.
   */
 ...
 * `hub-town` / `hub-mill` / `hub-dvm` (production) or
 * `hub-connector` / `pull:<image>` (other namespaces) — only the first
