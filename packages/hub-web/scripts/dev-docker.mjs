 ...
 * 1. Asserts .env.hub-dev exists (clear error if absent).
...
const envFile = join(repoRoot, '.env.hub-dev');

if (!existsSync(envFile)) {
  console.error(
    '\n[hub-web] Error: .env.hub-dev not found at:\n' +
    `  ${envFile}\n\n` +
    'Run `./scripts/hub-dev-infra.sh up` first.\n'
  );
  process.exit(1);
}
...
if (!existsSync(concurrently)) {
  console.error(
    '\n[hub-web] Error: concurrently binary not found at:\n' +
    `  ${concurrently}\n\n` +
...
proc.on('error', (err) => {
  console.error('[hub-web] failed to spawn concurrently:', err);
  process.exit(1);
});
