...
function withChainProviders(
  providers: TownhouseConfig['chainProviders']
): TownhouseConfig {
  return {
    ...getDefaultConfig(),
    network: 'custom',
...
describe('listSupportedSettlementAssets', () => {
  ...
  it('surfaces settlement-complete PRESET chains for network=testnet (no chains add)', () => {
    const cfg: TownhouseConfig = { ...getDefaultConfig(), network: 'testnet' };
    const ids = new Set(
  ...
  it('mainnet has no settlement-ready chains (contracts not deployed)', () => {
    const cfg: TownhouseConfig = { ...getDefaultConfig(), network: 'mainnet' };
