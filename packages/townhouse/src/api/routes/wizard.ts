...
const ALLOWED_WS_ORIGIN_HOSTS = new Set(['localhost', '127.0.0.1', '[::1]', '::1']);
...
export function registerWizardRoutes(
  ...
  app.post('/wizard/init', async (request, reply) => {
    if (state.mode === 'normal') {
      return reply.status(409).send({ code: 'wizard_already_completed', message: 'Setup is already complete.' });
    }
    ...
    if (state.initInFlight) {
      return reply.status(409).send({ code: 'init_in_flight', message: 'A setup is already in progress.' });
    }
    ...
    try {
      ...
      if (!body || typeof body !== 'object') {
        return reply.status(400).send({ code: 'invalid_request', message: 'Request body required.' });
      }
      ...
      if (typeof password !== 'string' || password.length === 0 || password.length > 256) {
        return reply.status(400).send({ code: 'password_invalid', message: 'password must be 1–256 characters.' });
      }
      if (password !== password.trim()) {
        return reply.status(400).send({ code: 'password_invalid', message: 'password cannot have leading or trailing whitespace.' });
      }
      if (password !== body.password_confirm) {
        return reply.status(400).send({ code: 'password_mismatch', message: 'Passwords do not match.' });
      }

      // Validate mnemonic_mode
      const mnemonicMode = body.mnemonic_mode;
      if (mnemonicMode !== 'generate' && mnemonicMode !== 'import') {
        return reply.status(400).send({ code: 'mnemonic_mode_invalid', message: 'mnemonic_mode must be "generate" or "import".' });
      }
      ...
      if (!mnemonic || typeof mnemonic !== 'string' || mnemonic.trim().length === 0) {
        return reply.status(400).send({ code: 'mnemonic_invalid', message: 'mnemonic is required.' });
      }
      if (!validateMnemonic(mnemonic.trim(), wordlist)) {
        return reply.status(400).send({ code: 'mnemonic_invalid', message: 'Invalid BIP-39 mnemonic.' });
      }
      ...
      if (body.backup_ack !== true) {
        return reply.status(400).send({
          ...
          message: 'You must confirm you have backed up your seed phrase before continuing.',
        });
      }
      ...
      if (!nodes || typeof nodes !== 'object') {
        return reply.status(400).send({ code: 'no_nodes_selected', message: 'nodes is required.' });
      }
      const atLeastOne = nodes.town?.enabled || nodes.mill?.enabled || nodes.dvm?.enabled;
      if (!atLeastOne) {
        return reply.status(400).send({ code: 'no_nodes_selected', message: 'At least one node must be enabled.' });
      }

      // Validate fee ranges
      if (nodes.town?.enabled && nodes.town.feePerEvent !== undefined) {
        if (!Number.isInteger(nodes.town.feePerEvent) || nodes.town.feePerEvent < 0 || nodes.town.feePerEvent > 1000) {
          return reply.status(400).send({ code: 'fee_out_of_range', message: 'nodes.town.feePerEvent must be 0–1000.' });
        }
      }
      if (nodes.mill?.enabled && nodes.mill.feeBasisPoints !== undefined) {
        if (!Number.isInteger(nodes.mill.feeBasisPoints) || nodes.mill.feeBasisPoints < 0 || nodes.mill.feeBasisPoints > 100) {
          return reply.status(400).send({ code: 'fee_out_of_range', message: 'nodes.mill.feeBasisPoints must be 0–100.' });
        }
      }
      if (nodes.dvm?.enabled && nodes.dvm.feePerJob !== undefined) {
        if (!Number.isInteger(nodes.dvm.feePerJob) || nodes.dvm.feePerJob < 0 || nodes.dvm.feePerJob > 100000) {
          return reply.status(400).send({ code: 'fee_out_of_range', message: 'nodes.dvm.feePerJob must be 0–100000.' });
        }
      }

      // Validate transport
      const transport = body.transport;
      if (!transport || (transport.mode !== 'direct' && transport.mode !== 'ator')) {
        return reply.status(400).send({ code: 'transport_invalid', message: 'transport.mode must be "direct" or "ator".' });
      }

      // Conflict checks — TOCTOU-resilient: the wallet.enc write below uses O_EXCL ('wx').
      if (existsSync(deps.walletPath)) {
        return reply.status(409).send({ code: 'wallet_already_exists', message: `A wallet already exists at ${deps.walletPath}. Delete it first.` });
      }
      if (existsSync(deps.configPath)) {
        return reply.status(409).send({ code: 'config_already_exists', message: `A config already exists at ${deps.configPath}. Delete it first.` });
      }
      ...
      const walletManager = new WalletManager({ encryptedPath: deps.walletPath });
      ...
      try { chmodSync(deps.walletPath, 0o600); } catch { /* best-effort */ }

      // Build + save config — if this throws, roll back the wallet write.
      let config: TownhouseConfig;
      try {
        config = buildConfigFromRequest(body as WizardInitRequest, deps.configPath);
        saveConfig(deps.configPath, config);
      } catch (err) {
        try { unlinkSync(deps.walletPath); } catch { /* best-effort */ }
        throw err;
      }
      ...
      if (onInit) {
        ...
        onInit(config, walletManager, profiles).catch((err: unknown) => {
          ...
          try { unlinkSync(deps.walletPath); } catch { /* best-effort */ }
          try { unlinkSync(deps.configPath); } catch { /* best-effort */ }
  ...
  app.get('/wizard/progress', { websocket: true }, (socket, req) => {
    ...
    if (!isAllowedWsOrigin(origin)) {
      try { socket.close(1008, 'origin_not_allowed'); } catch { /* best-effort */ }
      return;
    }
...
export function buildConfigFromRequest(
  ...
  if (request.nodes.town.enabled && request.nodes.town.feePerEvent !== undefined) {
    config.nodes.town.feePerEvent = request.nodes.town.feePerEvent;
  }

  config.nodes.mill.enabled = request.nodes.mill.enabled;
  if (request.nodes.mill.enabled && request.nodes.mill.feeBasisPoints !== undefined) {
