...
class MockWalletManagerNoInit {
  getNodeKeys() { throw new Error('Wallet not initialized'); }
  listKeys() { return []; }
}
...
function buildDeps(wallet: MockWalletManager | MockWalletManagerNoInit): ApiDeps {
  return {
...
describe('GET /nodes/:nodeId/deposit-addresses (AC-4, Story 21.11)', () => {
  ...
  it('mill returns evm + solana + mina families', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/nodes/mill/deposit-addresses' });
    ...
    const evmEntry = body.chains.find((c: { family: string }) => c.family === 'evm');
    expect(evmEntry.address).toBe(MOCK_MILL_KEYS.evmAddress);
    const solEntry = body.chains.find((c: { family: string }) => c.family === 'solana');
  ...
  it('town returns evm-only', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/nodes/town/deposit-addresses' });
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body);
  ...
  it('dvm returns evm-only', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/nodes/dvm/deposit-addresses' });
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body);
    expect(body.chains).toHaveLength(1);
    expect(body.chains[0].family).toBe('evm');
  ...
  it('returns 404 for unknown nodeId', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/nodes/unknown/deposit-addresses' });
  ...
  it('multi-instance: dev-mill-01 resolves to mill keys', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/nodes/dev-mill-01/deposit-addresses' });
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body);
    expect(body.chains).toHaveLength(3);
  });
