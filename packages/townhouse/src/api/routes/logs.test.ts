...
describe('GET /api/logs/stream', () => {
  ...
  it('emits a structured warning when no townhouse containers are running', async () => {
    const docker = makeFakeDocker([]);
    app = await buildAppWith(docker, makeTailFn({}));
    const res = await app.inject({ method: 'GET', url: '/api/logs/stream' });
    const body = res.body;
    ...
    expect(parsed.msg).toMatch(/no townhouse containers running/);
  });

  it('streams events from each running container', async () => {
    const docker = makeFakeDocker(['townhouse-town', 'townhouse-mill']);
