/**
 * Townhouse Dev Stack Smoke Test (Story 21.8.0, AC-6)
 *
 * Verifies the full Townhouse dev topology is healthy after
 * `./scripts/townhouse-dev-infra.sh up` completes.
 *
 * Prerequisites:
 *   ./scripts/townhouse-dev-infra.sh up   # writes .env.townhouse-dev
 *
 * Skip guards:
 *   SKIP_DOCKER=1   — skips the entire suite (sandbox environments)
 *   Missing .env.townhouse-dev — skips with a clear "run up first" message
 *
...
// Resolve workspace root relative to this file (packages/townhouse/src/__integration__)
const WORKSPACE_ROOT = join(import.meta.dirname, '..', '..', '..', '..');
const ENV_FILE = join(WORKSPACE_ROOT, '.env.townhouse-dev');
...
if (ENV_FILE_MISSING && !SKIP_DOCKER) {
  console.warn(
    '\n⚠️  .env.townhouse-dev not found — skipping Townhouse dev stack smoke test.\n' +
      '   Run `./scripts/townhouse-dev-infra.sh up` first, then re-run this test.\n'
  );
}
...
describe.skipIf(SKIP_DOCKER || ENV_FILE_MISSING)(
  'Townhouse dev stack smoke test',
  () => {
    ...
    beforeAll(() => {
      ...
      if (!connectorAdminUrl) {
        throw new Error(
          '.env.townhouse-dev is missing TOWNHOUSE_CONNECTOR_ADMIN_URL — was the file written by a current version of townhouse-dev-infra.sh?'
        );
      }
