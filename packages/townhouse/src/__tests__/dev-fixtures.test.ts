...
function validateMillConfigShape(
  ...
  expect(Array.isArray(cfg['swapPairs']), `${label}: swapPairs must be array`).toBe(true);
  const pairs = cfg['swapPairs'] as unknown[];
  expect(pairs.length, `${label}: swapPairs must be non-empty`).toBeGreaterThan(0);
  ...
  for (const pair of pairs) {
    ...
    expect(typeof from?.['assetCode'], `${label}: pair.from.assetCode`).toBe('string');
    expect(typeof from?.['assetScale'], `${label}: pair.from.assetScale`).toBe('number');
    expect(typeof from?.['chain'], `${label}: pair.from.chain`).toBe('string');
    expect(typeof to?.['assetCode'], `${label}: pair.to.assetCode`).toBe('string');
    expect(typeof to?.['assetScale'], `${label}: pair.to.assetScale`).toBe('number');
  ...
  expect(Array.isArray(cfg['chains']), `${label}: chains must be array`).toBe(true);
  const chains = cfg['chains'] as unknown[];
  expect(chains.length, `${label}: chains must be non-empty`).toBeGreaterThan(0);
  for (const c of chains) {
    expect(['evm', 'solana', 'mina'], `${label}: invalid chain kind "${c as string}"`).toContain(c);
  }
  ...
  for (const [chainKey, entries] of Object.entries(channels ?? {})) {
    expect(Array.isArray(entries), `${label}: channels.${chainKey} must be array`).toBe(true);
    for (const entry of entries as unknown[]) {
      const e = entry as Record<string, unknown>;
      expect(typeof e['channelId'], `${label}: channels.${chainKey}[*].channelId`).toBe('string');
      // cumulativeAmount and nonce are number in JSON (toBigInt handles conversion)
      expect(['number', 'string'].includes(typeof e['cumulativeAmount']), `${label}: cumulativeAmount type`).toBe(true);
      expect(['number', 'string'].includes(typeof e['nonce']), `${label}: nonce type`).toBe(true);
      // Non-zero for dev fixtures (confirms channel is seeded)
      expect(Number(e['cumulativeAmount']), `${label}: cumulativeAmount must be > 0`).toBeGreaterThan(0);
      expect(Number(e['nonce']), `${label}: nonce must be > 0`).toBeGreaterThan(0);
    }
  }
  ...
  for (const [chainKey, amt] of Object.entries(inventory ?? {})) {
    expect(['number', 'string'].includes(typeof amt), `${label}: inventory.${chainKey} type`).toBe(true);
    expect(Number(amt), `${label}: inventory.${chainKey} must be > 0`).toBeGreaterThan(0);
  }

  // relayUrls: non-empty array of strings starting with ws://
  const relayUrls = cfg['relayUrls'] as unknown[] | undefined;
  expect(Array.isArray(relayUrls), `${label}: relayUrls must be array`).toBe(true);
  expect((relayUrls ?? []).length, `${label}: relayUrls must be non-empty`).toBeGreaterThan(0);
  for (const url of relayUrls ?? []) {
    expect(typeof url, `${label}: relayUrl must be string`).toBe('string');
    expect((url as string).startsWith('ws://'), `${label}: relayUrl must start with ws://`).toBe(true);
  }
}
...
describe('dev-fixtures Mill config shape validation', () => {
  ...
  it('mill-01 swap pair is EVM↔Solana', () => {
    const cfg = loadFixture('mill-01.config.json');
    const pairs = cfg['swapPairs'] as Array<Record<string, Record<string, string>>>;
    const pair = pairs[0];
