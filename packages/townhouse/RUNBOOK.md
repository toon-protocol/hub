# Townhouse Operator Runbook — Multi-Chain On-Chain Settlement
...
A child node's route and peer relation are **dynamic connector state held in memory**, not persisted into the connector's static `connector.yaml` (which ships `peers: []`). `townhouse node add` registers the child at runtime via the connector admin API. When the connector process restarts, that runtime registration is gone. Until the child is re-registered:
...
// what the reconciler POSTs to /admin/peers per missing child:
{
  ...
  "url": "ws://townhouse-…-town:3000",           // child's internal BTP URL
...
(Source: `packages/townhouse/src/reconciler.ts` and `packages/townhouse/src/api/routes/nodes-lifecycle.ts` step 6 `register-peer`.)
...
This is a connector-side robustness gap (the monitor lacks an automatic transition out of `IN_PROGRESS` on a non-retryable failure), not a townhouse-side bug. The state lives entirely inside the connector process.
...
For all other (non-settlement) operational issues — boot failures, port conflicts, wallet/password problems — see the [README Troubleshooting table](./README.md#troubleshooting). For verbose logs on any failure, re-run the failing command with `DEBUG=townhouse:*`.
