 ...
 *
 * These tests verify:
 * - AC #2: `hub init` generates BIP-39 mnemonic and prompts backup
 * - AC #6: `hub wallet show` displays addresses without revealing secrets
...
function makeTempDir(): string {
  const dir = join(
    tmpdir(),
    `hub-wallet-cli-test-${randomBytes(8).toString('hex')}`
  );
...
`,
  ...
  // and no password. See docs/hub-mcp-design.md §3.
...
`,
  ...
  // emit a single JSON object on success. See docs/hub-mcp-design.md §2.
