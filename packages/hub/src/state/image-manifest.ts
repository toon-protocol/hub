/**
 * Image manifest reader for Hub (Story 46.2).
 *
 * `~/.hub/image-manifest.json` is materialized by `compose-loader.ts`
 * during `hub hs up`. It maps node types to their digest-pinned image
 ...
 * missing manifest; it means `hub hs up` was not run first.
