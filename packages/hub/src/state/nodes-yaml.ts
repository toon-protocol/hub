 ...
 *
 * `~/.hub/nodes.yaml` is the operator-managed source of truth for
 * enabled child nodes. The reconciler (see `../reconciler.ts`) converges
 * connector peer state to this file on every `hub hs up`.
 *
 ...
 * for `~/.hub/nodes.yaml`). Prevents partial-write corruption if the
