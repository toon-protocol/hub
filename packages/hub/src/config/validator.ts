/**
 * Runtime validation for Hub configuration.
 * Validates shape, narrows types, returns typed config or throws.
 */

import type { ChainProviderEntry, HubConfig } from './schema.js';
...
/**
 * Validate raw input and return a typed HubConfig.
...
export function validateConfig(raw: unknown): HubConfig {
  ...
  if (raw['network'] !== undefined) {
    ...
    network = raw['network'] as HubConfig['network'];
  }

  // endpoints (optional) — operator-supplied RPC URLs for network: 'custom'
  let endpoints: HubConfig['endpoints'];
