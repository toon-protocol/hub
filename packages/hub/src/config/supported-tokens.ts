 ...
 * presets) UNION any operator `chainProviders` from `hub chains add`. We
 ...
 */
function supportedChainIds(config: HubConfig): string[] {
  const ids: string[] = [];
  ...
  if (onChain.length === 0) {
    ...
    throw new UnsupportedSettlementError(
      `Unsupported settlement chain '${chainId}'. This deployment supports: ${chains}. ` +
        `Add a chain with 'hub chains add' or change 'network', then retry.`
    );
  }
