...
import type { HubConfig, ChainProviderEntry } from './config/schema.js';
 ...
 * `hub --version` so tooling that shells out (e.g. @toon-protocol/
 * hub-mcp's version-skew probe) has a real version to compare against.
 * Resolves `../package.json` relative to the module — package root from
 * `dist/cli.js`, and `packages/hub/package.json` under vitest/src.
 */
...
const HELP_TEXT = `hub — TOON node orchestrator

Usage:
  hub --version [--json]                    Print the package version (--json: { "version" })
  hub setup [--no-browser] [--port <n>] [--config-dir <dir>]  Run the first-run setup wizard
  hub init [--force] [--config-dir <dir>] [--password <pw>] [--preset <name>] [--network <mode>] [--yes] [--json]   Initialize config + wallet (set TOWNHOUSE_MNEMONIC + no password = config-only, no encrypted wallet)
  hub up [--transport direct|hs] [--dev] [--town] [--mill] [--dvm] [-c <path>] [--password <pw>]
  ...
  hub down [-c <path>] [--json]            Stop all nodes
  hub status [-c <path>] [--json]          Show node status
  hub urls [-c <path>] [--json]            Print BTP (write) + relay (read) URLs to share with clients
  hub metrics [-c <path>]                  Show connector metrics
  hub wallet show [--json] [--hex] [--paths] [-c <path>] [--password <pw>]  Show derived addresses
  hub wallet seed --confirm [-c <path>] [--password <pw>] [--json]    Print the BIP-39 seed phrase (password-gated, requires --confirm)
  hub credits buy --token <id> --amount <decimal> [--fee-multiplier <n>] [--quote-only] [--yes] [-c <path>] [--password <pw>]
                                                 Buy Arweave upload credits (token: eth|sol|pol|base-eth|base-usdc|usdc-eth|usdc-pol)
  hub credits balance --token <id> [-c <path>] [--password <pw>]  Show Turbo credit balance for the funding address
  hub hs up [--password <pw>] [--skip-preflight] [-c <path>]  Boot/enable hidden-service mode (opt-in, anonymous .anon apex) (launches dashboard TUI in TTY mode)
  hub hs enable [--password <pw>] [-c <path>] [--json]   Switch a running direct apex to hidden-service mode (down direct → up HS; --json emits NDJSON boot steps)
  hub hs down [--rotate-keys] [-c <path>]               Stop apex (--rotate-keys deletes .anyone keypair)
  hub node add [<type>] [--json] [-c <path>]    Provision a child node (default: town)
  hub node remove <id> [--yes] [--json] [-c <path>]   Deprovision a child node
  hub node list [--json] [-c <path>]            List provisioned nodes
  hub chains list [--json] [-c <path>]          List configured settlement chains (EVM/Solana/Mina)
  hub chains add --chain-type <evm|solana|mina> --chain-id <id> [fields] [-c <path>]   Add/update a settlement chain
  hub chains remove <chainId> [-c <path>]       Remove a settlement chain
  hub channels [--json]                    Show open payment channels
  hub logs <node-id> [-f|--follow] [--lines N] [--json]   Tail logs for a node (Ctrl-C to stop)
  hub peer <id> [--json]                   Show per-peer detail card
  hub health [--json]                      Probe apex/api/nodes/.anyone health
...
export interface CliHsOverrides {
   ...
   * `BootReconciler` against `~/.hub/{nodes.yaml,reconciler.log}`.
   */
  createReconciler?: (
...
const DEFAULT_CONFIG_DIR = join(homedir(), '.hub');
...
async function handleInit(
  ...
  // demo init. Preset writes the same HubConfig shape, so the rest of
  ...
  if (preset === 'demo') {
    ...
    // getDefaultConfig() hardcodes ~/.hub/wallet.enc; tests and non-default
  ...
  if (json) {
    ...
    // (docs/hub-mcp-design.md §3). Zero key material afterwards.
    console.log(
      JSON.stringify({
        created: true,
        configPath,
...
async function handleSetup(
  ...
  // says "already initialized → run `hub up`", up then errors that the
  // wallet is missing). Guide them to clean up and re-run setup instead.
  if (existsSync(configPath) && existsSync(walletPath)) {
    console.log('Already initialized — run `hub up` to start your nodes');
    return;
  }
  if (existsSync(configPath) && !existsSync(walletPath)) {
    console.error(
      `Found ${configPath} but no wallet at ${walletPath}.\n` +
        `Delete the orphan config and re-run \`hub setup\`, or restore the wallet from backup.`
    );
    process.exitCode = 1;
    return;
  }
...
/**
 * Extended `hub wallet show` (epic-49, Phase 3).
 *
...
async function handleWalletShow(
  ...
  if (!walletManager) {
    ...
    if (!result) {
      console.error(
        'No wallet found. Run `hub init` first (or set TOWNHOUSE_MNEMONIC).'
      );
      process.exitCode = 1;
      return;
    }
  ...
  try {
    ...
    console.log('Tip: hub wallet show --json   for scripting');
    console.log('     hub wallet show --hex    to see raw hex pubkeys');
    console.log('     hub wallet show --paths  to see derivation paths');
    console.log(
      '     hub credits buy --token sol --amount <n>  to fund Arweave uploads'
    );
  } finally {
...
/**
 * `hub wallet seed --confirm` (epic-49, Phase 3).
 *
...
async function handleWalletSeed(
  ...
  try {
    ...
    console.log(' [!] Anyone who sees this seed owns your hub identity.');
    ...
    console.log('');
    console.log(
      'This is the same 12 words shown at `hub init`. Storing them elsewhere is your responsibility.'
    );
  } finally {
    walletManager.lock();
  }
 ...
 * password indirection — see docs/hub-mcp-design.md §3.
 *
...
/**
 * Handle `hub credits buy --token <id> --amount <decimal> [...]`.
 *
...
async function handleCreditsBuy(
  ...
  if (!tokenRaw || !amountRaw) {
    console.error(
      'Usage: hub credits buy --token <id> --amount <decimal> [--fee-multiplier <n>] [--credit-destination <addr>] [--quote-only] [--yes]'
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (!wallet) {
    ...
    if (!loaded) {
      console.error(
        `No wallet found at ${walletPath}. Run \`hub init\` first (or set TOWNHOUSE_MNEMONIC).`
      );
      process.exitCode = 1;
      return;
    }
...
/**
 * Handle `hub credits balance --token <id>`.
 *
...
async function handleCreditsBalance(
  ...
  if (!tokenRaw) {
    console.error(
      'Usage: hub credits balance --token <id> [-c <path>] [--password <pw>]'
    );
    process.exitCode = 1;
    return;
  }
...
async function handleUp(
  ...
  // encrypted wallet + password. P1 / docs/hub-mcp-design.md §3.
  let walletManager: WalletManager | undefined =
    (await tryEnvMnemonicWallet(walletPath)) ?? undefined;
  let walletPassword: string | undefined;
  if (!walletManager) {
    if (!existsSync(walletPath)) {
      console.error(
        `Wallet not found at ${walletPath}. Run \`hub setup\` first (or restore your wallet backup), or set TOWNHOUSE_MNEMONIC.`
      );
      process.exitCode = 1;
      return;
    }
  ...
  if (profiles.includes('dvm')) {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.warn(
        `[hub up] AR pre-warm failed (non-fatal, orchestrator will retry): ${msg}`
      );
    }
  }
  ...
  if (
    ...
    console.warn(
      '[hub] WARN: TURBO_TOKEN is not set — Arweave DVM (kind:5094) free-tier (<100KB) uploads still work, but larger/paid uploads will fail.'
    );
    console.warn(
      '[hub] Pass `hub node add dvm --turbo-token <arweave-jwk-json>` (HS mode) or export TURBO_TOKEN before `hub up` to enable full uploads.'
    );
  }
  ...
  try {
    ...
    if (walletManager) {
      ...
      if (!dryRun) {
        ...
        console.log(
          `\n[Hub API] listening on http://${host ?? '127.0.0.1'}:${port ?? 9400}`
        );
        console.log(
...
/** Hub API URL for HS mode (inside the hub-api container). */
...
/**
 * Auto-rebind provisioned child nodes from `~/.hub/nodes.yaml` after the
...
async function rebindAndReconcileChildren(opts: {
  ...
  // `docker compose up` subprocess (else the bind spec is `:/.hub` → error).
...
async function collectApexImageRefs(configDir: string): Promise<string[]> {
  ...
  try {
    ...
    if (
      ...
      isSyntheticDigest(manifest.images['hub-api'].digest)
    ) {
      return [];
    }
    return [
      `${manifest.images.connector.name}@${manifest.images.connector.digest}`,
      `${manifest.images['hub-api'].name}@${manifest.images['hub-api'].digest}`,
    ];
  } catch {
    return [];
  }
 ...
 * re-running `hub hs up` against a running node re-attaches the dashboard
...
/**
 * P2b — emit one NDJSON boot-progress step on stdout when `hub up --json`
 * / `hs up --json` is active. The hub-mcp server's `hub_up_status`
...
async function handleHsUp(
    ...
    try {
      ...
      const detail =
        preflightErr instanceof Error
          ...
          : String(preflightErr);
      console.error(
        `[hub hs up] port preflight skipped (non-fatal): ${detail}`
      );
    }
  }
  ...
  // See P1 / docs/hub-mcp-design.md §3.
  const walletPath = config.wallet.encrypted_path;
  let walletManager: WalletManager | undefined =
    (await tryEnvMnemonicWallet(walletPath)) ?? undefined;
  // Hoisted: injected into the hub-api container env below. Undefined in
  // TOWNHOUSE_MNEMONIC mode (the container then receives TOWNHOUSE_MNEMONIC).
  let resolvedPassword: string | undefined;
  if (!walletManager) {
    if (!existsSync(walletPath)) {
      console.error(
        `Wallet not found at ${walletPath}. Run \`hub init\` first (or set TOWNHOUSE_MNEMONIC).`
      );
      process.exitCode = 1;
      return;
    }
  ...
  try {
    ...
    const { composePath } = materialize('hs', { hubHome: configDir });
    ...
    // Inject env vars that Docker Compose interpolates in hub-hs.yml:
    //   TOWNHOUSE_HOME — operator's config dir; replaces hardcoded `~/.hub`
    ...
    //   TOWNHOUSE_WALLET_PASSWORD — required by hub-api service
    //   TOWNHOUSE_UID — run hub-api as the host user so bind-mounted
    //     ~/.hub files (rw------- 600) are readable inside the container
    ...
    //     from hub-api fail with `connect EACCES /var/run/docker.sock`.
    let dockerSockGid = 0;
    try {
    ...
    const prevHubHome = process.env['TOWNHOUSE_HOME'];
    const prevWalletPassword = process.env['TOWNHOUSE_WALLET_PASSWORD'];
    const prevHubUid = process.env['TOWNHOUSE_UID'];
    ...
    process.env['TOWNHOUSE_UID'] = String(process.getuid?.() ?? 1000);
    // Inject the wallet dir as an absolute host path so the hub-api
    ...
    try {
      for (let attempt = 1; attempt <= MAX_ANON_RETRIES; attempt++) {
        try {
          ...
          if (isAnonBootstrapTimeout(err) && attempt < MAX_ANON_RETRIES) {
            console.error(
              `[hub hs up] ATOR bootstrap timed out (attempt ${attempt}/${MAX_ANON_RETRIES}) — retrying...`
            );
      ...
      if (prevHubHome === undefined) {
        delete process.env['TOWNHOUSE_HOME'];
      } else {
        process.env['TOWNHOUSE_HOME'] = prevHubHome;
      }
      ...
      if (prevHubUid === undefined) {
        delete process.env['TOWNHOUSE_UID'];
      } else {
        process.env['TOWNHOUSE_UID'] = prevHubUid;
      }
    ...
    // kind:10032) on the next boot, and `hub urls` shows it immediately.
    let relayHostname: string | undefined;
    if (
      ...
      try {
        ...
        if (relayHostname) {
          console.error(
            `[hub hs up] relay hidden service published: wss://${relayHostname}/`
          );
        } else {
          console.error(
            '[hub hs up] relay hidden service started; hostname not resolved yet (will appear on next `hs up` / `hub urls`)'
          );
        }
        ...
        const detail =
          relayErr instanceof Error
            ...
            : String(relayErr);
        console.error(
          `[hub hs up] relay hidden service error (non-fatal): ${detail}`
        );
      }
    }
 ...
 *
 * Boots the apex (connector + hub-api) + any requested children with NO
 ...
 *
 * This is reachable ONLY via `hub up --transport direct` (an explicit
...
async function handleDirectUp(
  ...
  // The direct-default `hub up` MUST NOT clobber an operator who is
  ...
  if (detectExistingHsConfig(configDir)) {
    console.error(
      'Existing hidden-service apex detected (connector.yaml has anon.enabled: true).\n' +
        '`hub up` boots a direct-BTP apex and would downgrade your HS deployment.\n' +
        '  • To keep hidden-service mode:  hub hs up\n' +
        '  • To switch to direct BTP:      hub hs down --rotate-keys && hub up'
    );
    process.exitCode = 1;
    return;
  }
    ...
    try {
      ...
      const detail =
        preflightErr instanceof Error
          ...
          : String(preflightErr);
      console.error(
        `[hub up --transport direct] port preflight skipped (non-fatal): ${detail}`
      );
    }
  }
  ...
  try {
    ...
    const materialize =
      hsOverrides?.materializeComposeTemplate ?? materializeComposeTemplate;
    const { composePath } = materialize('direct', { hubHome: configDir });
...
/**
 * `hub hs enable` — switch an already-running DIRECT deployment to HS.
 *
...
async function handleHsEnable(
  ...
  if (detectExistingHsConfig(configDir)) {
    if (json) {
      ...
      console.log(
        'Hidden-service apex already configured. Use `hub hs up` to (re)attach.'
      );
    }
    return;
  }
  ...
  try {
    ...
    if (json) {
      ...
      console.warn(
        `[hub hs enable] direct stack teardown skipped (non-fatal): ${detail}`
      );
    }
  }
...
/** Atomically write ~/.hub/host.json (AC #6). */
  ...
  const content = JSON.stringify(
    {
      ...
      hubApiUrl: HS_TOWNHOUSE_API_URL,
...
/**
 * Stop the apex via `hub hs down`.
 * Default: preserves the hub-hs-anon volume (stable .anyone address).
...
async function handleHsDown(
  ...
  // fallbacks for the wallet dir use a literal `~/.hub` which Docker
  ...
  if (rotateKeys) {
    ...
    // Run `docker compose down -v` to remove volumes (including hub-hs-anon).
  ...
  // Default: preserve volumes (hub-hs-anon survives → same hostname next hs up).
  const orchestratorFactory =
    hsOverrides?.createOrchestrator ??
    ((
      d: Docker,
 ...
 */
const CHAINS_HELP = `hub chains — configure settlement chains (connector chainProviders)

The connector settles ILP payment claims on these chains. Changes take effect
on the next 'hub hs down && hub hs up'.

Usage:
  hub chains list [--json] [-c <path>]
  hub chains add  --chain-type <evm|solana|mina> --chain-id <id> [fields] [-c <path>]
  hub chains remove <chainId> [-c <path>]
...
function buildChainProviderFromFlags(f: ChainsFlags): ChainProviderEntry {
  ...
  // `--key-id` is OPTIONAL: when omitted, `hub hs up` fills it with the
...
/**
 * `hub chains <list|add|remove>` — edit the connector settlement chains
  ...
  switch (action) {
    case 'list': {
      ...
      if (providers.length === 0) {
        ...
        console.log(
          'Add one with:  hub chains add --chain-type evm --chain-id evm:base:8453 ...'
        );
        return;
      }
    ...
    case 'add': {
      ...
      console.log('Apply with:  hub hs down && hub hs up');
      return;
    }
    case 'remove': {
      if (!chainIdArg) {
        console.error('Usage: hub chains remove <chainId>');
        process.exitCode = 1;
        return;
      }
...
export async function main(
  ...
  const { values, positionals } = parseArgs({
    ...
    options: {
      ...
      // Phase 3: `hub up` defaults to a direct-BTP apex + children.
  ...
  // `hub --version` / `hub version` — print the package version and
  ...
  // Handle `hub node <verb> --help` before the global --help check so
  ...
  switch (command) {
    ...
    case 'wallet': {
      ...
      if (subCommand === 'show') {
        ...
        console.error(
          'Usage:\n' +
            '  hub wallet show [--json] [--hex] [--paths] [-c <path>] [--password <pw>]\n' +
            '  hub wallet seed --confirm [-c <path>] [--password <pw>]'
        );
        process.exitCode = 1;
      }
      break;
    ...
    case 'credits': {
      ...
      if (subCommand === 'buy') {
        ...
        console.error(
          'Usage:\n' +
            '  hub credits buy --token <id> --amount <decimal> [--fee-multiplier <n>] [--quote-only] [--yes] [-c <path>] [--password <pw>]\n' +
            '  hub credits balance --token <id> [-c <path>] [--password <pw>]'
        );
        process.exitCode = 1;
      }
      break;
    ...
    case 'urls': {
      ...
      if (values['json'] === true) {
        ...
        console.log('');
        console.log(
          `  Write (pay-to-publish, BTP):  ${btpUrl ?? '(apex not running — run `hub up` / `hs up`)'}`
        );
        if (ilpDestination) {
    ...
    case 'up': {
      ...
      // `--transport hs` is a synonym for `hub hs up`.
      if (transport === 'hs') {
        await handleHsUp(configPath, configDir, config, docker, {
    ...
    case 'hs': {
      ...
      if (action === 'up') {
        ...
        console.error(
          'Usage: hub hs <up|enable|down> [--rotate-keys] [--password <pw>] [-c <path>]'
        );
        process.exitCode = 1;
      }
      break;
    ...
    case 'chains': {
      ...
      if (action === 'supported') {
        ...
        if (values.json === true) {
          ...
          console.log(
            'No supported settlement chains. Set `network` (mainnet/testnet/devnet) or run `hub chains add`.'
          );
        } else {
          console.log(
...
// process.argv[1] can be a symlink (e.g. node_modules/.bin/hub created by
