...
/**
 * Local-devnet fallback URLs (CLAUDE.md "Hub Dev Stack" 28xxx range).
...
export const DEMO_DETERMINISTIC_PASSWORD =
  'hub-demo-INSECURE-do-not-use-in-prod';
...
/**
 * Build the full HubConfig for `--preset=demo`.
 *
...
export function buildDemoConfig(
  ...
  return {
    ...
    transport: {
      // 'direct' for the demo because `hub up` doesn't bring up a
      ...
      // to 'hs' once the sidecar story lands in hub `up`.
      mode: 'direct',
    },
    api: {
      port: 9400,
