...
export interface ComposeLoaderOptions {
  /** Override default `~/.hub/` write target. Used by tests. */
  hubHome?: string;
...
// Reject hubHome paths that target system directories. Internal callers
// (CLI, API, Story 45.4) pass `~/.hub` or test tmpdirs; an attacker
// reaching this code path with `hubHome: '/etc'` would otherwise write
// `/etc/compose/hub-hs.yml` and `chmod /etc 0o700`. The list is a
...
function assertValidHubHome(home: string): void {
  if (!home) {
    throw new ComposeLoaderError(
      'hubHome resolved to an empty path. Set $HOME or pass options.hubHome explicitly.'
    );
  }
  if (!isAbsolute(home)) {
    throw new ComposeLoaderError(
      `hubHome must be an absolute path; got '${home}'.`
    );
  }
  if (home === '/' || home === '\\') {
    throw new ComposeLoaderError(
      `hubHome must not be the filesystem root; got '${home}'. ` +
  ...
  for (const prefix of SYSTEM_PATH_PREFIXES) {
    if (home === prefix || home.startsWith(prefix + '/')) {
      throw new ComposeLoaderError(
        `hubHome must not target a system directory; got '${home}'. ` +
...
export function loadComposeTemplate(
  ...
  const composePath = join(distDir, 'compose', `hub-${profile}.yml`);
...
/**
 * Writes the resolved compose YAML to `<hubHome>/compose/<profile>.yml`
 * and copies `dist/image-manifest.json` to `<hubHome>/image-manifest.json`.
...
export function materializeComposeTemplate(
  ...
  assertValidProfile(profile);
  const home = options.hubHome || join(homedir(), '.hub');
  assertValidHubHome(home);
  ...
  // Refuse to chmod symlink targets — operator may have placed `~/.hub`
  ...
  const composePath = join(composeDir, `hub-${profile}.yml`);
