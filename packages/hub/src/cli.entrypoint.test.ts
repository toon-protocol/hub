 ...
 *
 * npm/npx install the `hub` bin as a SYMLINK
 * (node_modules/.bin/hub -> ../@toon-protocol/hub/dist/cli.js).
...
describe.skipIf(!built)('cli entrypoint (symlink invocation)', () => {
  ...
  beforeAll(() => {
    tmp = mkdtempSync(join(tmpdir(), 'hub-bin-'));
    link = join(tmp, 'hub');
    // Mimic node_modules/.bin/hub -> dist/cli.js
