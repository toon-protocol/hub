/**
 * Hub CLI Lifecycle Integration Tests (Story 21.16, AC-3)
 *
 * Coverage map (AC-14):
...
if (!shouldRun) {
  console.warn(
    '\n⚠️  Skipping Hub CLI lifecycle integration tests.\n' +
      '   Set RUN_DOCKER_INTEGRATION=1 and ensure SKIP_DOCKER is unset.\n' +
      '   Pre-warm the image cache first: bash scripts/hub-test-infra.sh up\n'
  );
}

/** Run `hub-test-infra.sh down` best-effort; swallows errors. */
...
/** List running containers whose names start with 'hub-'. */
function listHubContainers(): string[] {
  const out = execSync(
    'docker ps --filter name=hub- --format "{{.Names}}"',
    { encoding: 'utf-8', timeout: 10_000 }
  );
  return out.trim().split('\n').filter(Boolean);
}

/** Check if the hub-net Docker network exists. */
function networkExists(): boolean {
  const out = execSync(
    'docker network ls --filter name=hub-net --format "{{.Name}}"',
    { encoding: 'utf-8', timeout: 10_000 }
  );
  return out.trim() === 'hub-net';
}
...
describe.skipIf(!shouldRun)(
  'Hub full lifecycle: init + up (all nodes) + SIGTERM (T-079, T-080, T-083, R-004, X-001)',
  () => {
    let tmpDir: string;
    ...
    beforeAll(async () => {
      // X-002: fresh config dir — never reuses ~/.hub across test runs
      tmpDir = mkdtempSync(join(tmpdir(), 'hub-test-lifecycle-'));

      // Reset any orphan Docker state from prior crashed runs
      infraDown();

      // Run `hub init` synchronously (it exits on its own)
      ...
      if (initCode !== 0) {
        throw new Error(
          `hub init exited with code ${initCode}. stdout: ${initResult.stdout.join('')}`
        );
      }

      // Spawn `hub up --town --mill --dvm` (long-running subprocess)
      upProcess = runCli('up', {
        configDir: tmpDir,
        password: TEST_PASSWORD,
    ...
    it('T-079 / X-001: 4 containers are running after init + up (all nodes)', () => {
      const containers = listHubContainers();
      const expected = [
        'hub-connector',
        'hub-town',
        'hub-mill',
    ...
    // ── T-083 / R-004 ─────────────────────────────────────────────────────────
    it('T-083 / R-004: after SIGTERM, all hub-* containers + hub-net removed', async () => {
      ...
      const containers = listHubContainers();
      const hubContainers = containers.filter((n) =>
        [
          'hub-connector',
          'hub-town',
          'hub-mill',
      ...
      expect(
        hubContainers,
        `Expected all hub-* containers to be removed after SIGTERM, found: ${JSON.stringify(hubContainers)}`
      ).toHaveLength(0);

      expect(
        networkExists(),
        'Expected hub-net to be removed after SIGTERM'
      ).toBe(false);
    }, 60_000);
  }
);
...
describe.skipIf(!shouldRun)('Single-node Mill-only operation (T-084)', () => {
  ...
  beforeAll(async () => {
    tmpDir = mkdtempSync(join(tmpdir(), 'hub-test-mill-'));
    infraDown();
  ...
  it('T-084: docker ps shows hub-connector + hub-mill; NOT town or dvm', () => {
    const containers = listHubContainers();
    expect(containers).toContain('hub-connector');
    expect(containers).toContain('hub-mill');
    expect(containers).not.toContain('hub-town');
    expect(containers).not.toContain('hub-dvm');
  }, 30_000);
});
