/**
 * Hub Config Propagation Integration Test (Story 21.16, AC-4)
 *
 * Coverage map (AC-14):
 ...
 *   4. Poll docker inspect hub-connector --format '{{.State.StartedAt}}'
 ...
 *
 * Skip guards: same as hub-cli-lifecycle.test.ts (AC-3, AC-5).
...
if (!shouldRun) {
  console.warn(
    '\n⚠️  Skipping Hub config propagation integration test.\n' +
      '   Set RUN_DOCKER_INTEGRATION=1 and ensure SKIP_DOCKER is unset.\n'
  );
}

/** Run `hub-test-infra.sh down` best-effort. */
...
function connectorStartedAt(): string {
  return execSync(
    "docker inspect hub-connector --format '{{.State.StartedAt}}'",
...
describe.skipIf(!shouldRun)(
  ...
  () => {
    ...
    beforeAll(async () => {
      tmpDir = mkdtempSync(join(tmpdir(), 'hub-test-config-'));
      infraDown();
