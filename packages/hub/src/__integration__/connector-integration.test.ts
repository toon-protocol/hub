...
describe.skipIf(!shouldRun)('Connector Integration (requires Docker)', () => {
  ...
  describe('startup sequence (T-017)', () => {
    ...
    it('connector and Town are on same Docker network', async () => {
      ...
      const hubNet = networks.find(
        (n: { Name: string }) => n.Name === 'hub-net'
      );
      expect(hubNet).toBeDefined();

      const netInfo = await docker.getNetwork(hubNet!.Id).inspect();
      const containerNames = Object.values(netInfo.Containers || {}).map(
        (c: any) => c.Name
      );
      expect(containerNames).toContain('hub-connector');
      expect(containerNames).toContain('hub-town');
    }, 30000);
  });
