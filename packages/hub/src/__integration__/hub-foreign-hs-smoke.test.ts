/**
 * Live smoke gate — Foreign TOON Client → Hub HS (.anyone) Loop (Story 49.1)
 *
 * Proves that an in-process ToonClient (the "foreign client", operator B) can:
 *   1. Establish a BTP/WS connection to a real `hub hs up` apex via real .anyone transport
 ...
 *   - hub hs up (apex cold-boot):            ~5 min
...
// swept `hub-hs-mill-data` and `hub-hs-dvm-data` (created by adjacent
  ...
  if (
    ...
    throw new Error(
      `dist/image-manifest.json missing images.connector{name,digest}. ` +
        `Re-run \`gh run download <id> --name image-manifest -D packages/hub/dist/\`.`
    );
  }
  ...
  if (bound.length > 0) {
    throw new Error(
      `Cannot start HS apex: ports already bound: ${bound.join(', ')}. ` +
        `Stop any concurrent hub stack and re-run.`
    );
  }
}
...
async function startBConnector(configYaml: string): Promise<string> {
  ...
  // - bConfigDir uses mkdtempSync (was fixed `tmpdir()/hub-foreign-b-config` → collisions)
  ...
  const bConfigDir = mkdtempSync(join(tmpdir(), 'hub-foreign-b-config-'));
...
describe.skipIf(!shouldRun)(
  'hub foreign HS smoke — real ToonClient → .anyone apex (Story 49.1)',
  () => {
    let tmpDirA: string;
    ...
    beforeAll(async () => {
      ...
      // (CLI_BIN resolves to packages/hub/dist/cli.js per _test-helpers.ts.)
      const thisFile = fileURLToPath(import.meta.url);
      ...
      tmpDirA = mkdtempSync(join(tmpdir(), 'hub-foreign-A-'));

      // Step 7a: hub init A. Pass 2 code review (P13): renumbered from
      ...
      // Step 7b: hub hs up A (apex cold-boot — 5 min cold budget)
      const up = runCli('hs', {
        configDir: tmpDirA,
        password: TEST_PASSWORD,
        env: { TOWNHOUSE_WALLET_PASSWORD: TEST_PASSWORD },
      ...
      // Wait 10s for the hub-api to fully initialize Docker access before adding.
      await sleep(10_000);
      console.log(
      ...
      if (addTownCode !== 0) {
        console.warn(
          `[49.1] hub node add town exited ${addTownCode} — stdout: ${addTownStdout.slice(0, 500)}. ` +
    ...
    afterAll(async () => {
      ...
      try {
        const dumpPath = join(
          tmpdir(),
          `hub-foreign-hs-smoke-logs-${Date.now()}.txt`
        );
        const lines: string[] = [];
      ...
      try {
        ...
        // Pass 2 code review (P2): wildcard sweep per AC #5 "ALL hub-foreign-*
        // containers." Catches partial-cleanup leftovers from prior crashed runs.
        try {
          const orphans = execSync(
            `docker ps -aq --filter "name=hub-foreign-"`,
        ...
        try {
          const orphanVols = execSync(
            `docker volume ls -q --filter "name=hub-hs-town-"`,
            { encoding: 'utf-8', timeout: 10_000 }
          ).trim();
          if (orphanVols) {
    ...
    it("A's peer-type resolver tags B's pubkey as 'external' (AC #4)", async () => {
      ...
      if (!primaryPassed) {
        ...
        // Path: packages/hub/src/registry/peer-type-resolver.ts
    ...
    it('host.json has correct schema (hostname + connectorAdminUrl + hubApiUrl)', () => {
      ...
      expect(json.hubApiUrl).toBe('http://127.0.0.1:28090');
