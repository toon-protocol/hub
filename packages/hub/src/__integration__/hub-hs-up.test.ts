 ...
 *
 * Boots the real apex stack via the CLI binary (`hub hs up`) and asserts:
 * - exactly two containers: connector + hub-api
 ...
 * - hs down preserves hub-hs-anon volume
...
describe.skipIf(!shouldRun)('hub hs up — real CLI apex boot', () => {
  let tmpDir: string;

  beforeAll(async () => {
    tmpDir = mkdtempSync(join(tmpdir(), 'hub-hs-cli-'));
  ...
  afterAll(async () => {
    // Best-effort cleanup
    try {
      const composePath = join(tmpDir, 'compose', 'hub-hs.yml');
  ...
  it('exactly two containers running: connector + hub-api', () => {
  ...
  it('hs down preserves hub-hs-anon volume', async () => {
