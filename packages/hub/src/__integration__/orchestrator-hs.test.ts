 ...
 *     -- orchestrator-hs
 *
 * First run pulls connector + hub-api images (~2-3 min cold cache).
 */
...
describe.skipIf(!shouldRun)(
  ...
  () => {
    ...
    beforeAll(async () => {
      tmpDir = mkdtempSync(join(tmpdir(), 'hub-hs-orch-'));
      ...
      // Pass a real HubConfig (NOT undefined) — waitForHsHostname reads
    ...
    it('exactly two containers running: connector + hub-api', () => {
      ...
      expect(names).toEqual(['hub-hs-api', 'hub-hs-connector']);
    }, 10_000);
    ...
    it('down() stops containers but preserves hub-hs-anon volume', async () => {
      await orch.down();
      const containers = execSync(
        'docker ps -a --filter name=hub-hs- --format "{{.Names}}"',
        { encoding: 'utf-8' }
      );
      expect(containers.trim()).toBe('');
      const volumes = execSync(
        'docker volume ls --filter name=hub-hs-anon --format "{{.Name}}"',
        { encoding: 'utf-8' }
      );
      expect(volumes.trim()).toBe('hub-hs-anon');
    }, 60_000);
  }
);
