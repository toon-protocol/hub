 ...
 *
 * Drives the real `hub node add|list|remove` CLI against the real
 * hub-api container (started via `hub hs up`) and asserts the
 * happy-path 5-step sequence completes end-to-end:
 *
 *   1. `hub node add town`  → 201, container running, nodes.yaml written
 *   2. `hub node list`      → 1 node, status connected (poll up to 30 s)
 *   3. `hub node remove <id>` → 200, container gone, nodes.yaml empty
 *   4. `hub node list`      → 0 nodes
 *   5. Re-run `hub hs up`   → idempotency probe, hostname unchanged,
 *                                    hub-hs-anon volume preserved
 *
 * Prerequisites:
...
const HS_TOWN_NAME = 'hub-hs-town';
const HS_ANON_VOLUME = 'hub-hs-anon';
// hub-api does not expose `/health`; poll `/api/nodes` instead — it's a
...
// to avoid a shared mutation surface). Mirrors hub-hs-up.test.ts:54-91.

function dockerPs(): string[] {
  const out = execSync(
    `docker ps --filter name=hub-hs- --format "{{.Names}}"`,
    { encoding: 'utf-8' }
  );
...
function cleanupContainersAndVolumes(): void {
  try {
    execSync(
      `docker ps -aq --filter name=hub-hs- | xargs -r docker rm -f`,
      { stdio: 'pipe' }
    );
  } catch {
    /* best-effort */
...
describe.skipIf(!shouldRun)(
  'hub node lifecycle E2E — real CLI against real Docker (Story 46.4)',
  () => {
    let tmpDir: string;
    ...
    beforeAll(async () => {
      ...
      tmpDir = mkdtempSync(join(tmpdir(), 'hub-node-e2e-'));
      ...
      // 2. hub hs up — cold-boot path; exits 0 after "Apex live at ..." is
      ...
      // 4. Wait for hub-api /api/nodes to become 200 — guards AC #1
      //    step 1 against racing with the API server's boot.
      await waitForUrl(HS_API_HEALTH_URL, {
        maxMs: 30_000,
        label: 'hub-api /api/nodes',
      });
    }, 480_000);

    afterAll(async () => {
      // Best-effort `hub hs down` to drain via the real CLI path.
      try {
    ...
    it('node list shows the Town node as active', async () => {
      ...
      // connected state, mirror hub-cli-lifecycle.test.ts:156-165.
