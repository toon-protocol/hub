 ...
 *   bash scripts/hub-test-infra.sh up  (warms image cache)
...
const HS_VOLUMES = ['hub-hs-anon', 'hub-hs-town-data'] as const;

const B_CONNECTOR_NAME = 'hub-foreign-b-connector';
const B_ANON_VOLUME = 'hub-foreign-b-anon';
const B_SOCKS5_URL = 'socks5h://127.0.0.1:9050';
const B_BTP_PORT = 3002;
const B_HEALTH_PORT = 8082;

const DVM_CONTAINER_NAME = 'hub-dvm';
const _DVM_ANON_VOLUME = 'hub-dvm-anon';
...
const MILL_CONTAINER_NAME = 'hub-hs-mill';
...
// /api/earnings requires hub-api RC5+; older HS images return 404.
...
function cleanupAll(): void {
  // 'hub-hs-town' is already in HS_CONTAINER_NAMES — do not append separately.
...
// DVM runs as a Docker container on hub-hs-net.
...
async function startDvm(): Promise<void> {
  ...
  // Launch DVM container on hub-hs-net so it can reach the connector at its bridge IP.
  ...
  // (on hub-hs-net) can reach it at hsNetGw:3300 via localDelivery.handlerUrl.
  execSync(
    `docker run -d \
...
function buildTestMillConfig(connectorBtpUrl: string): object {
  return {
    ...
    parentAuthToken: '',
    // Relay for kind:10032 advertisement (within hub-hs-net).
    relayUrls: ['ws://hub-hs-town:7100'],
  };
}
...
async function startMill(bConfigDir: string): Promise<void> {
  ...
  execSync(
    `docker run -d \
      ...
      -e MILL_RELAYS=ws://hub-hs-town:${TOWN_RELAY_WS_PORT} \
...
describe.skipIf(!shouldRun)(
  'hub DVM Arweave E2E — .anyone + kind:5094 (Story 49-5)',
  () => {
    let tmpDirA: string;
    ...
    beforeAll(async () => {
      ...
      bConfigDir = mkdtempSync(join(tmpdir(), 'hub-49-5-b-'));
      ...
      // ── Init + hs up A ──────────────────────────────────────────────────────
      tmpDirA = mkdtempSync(join(tmpdir(), 'hub-49-5-a-'));
      ...
      if (initCode !== 0) throw new Error(`hub init exited ${initCode}`);
      ...
      if (upCode !== 0) throw new Error(`hub hs up exited ${upCode}`);
      ...
      // Get the hub-hs-net gateway (connector container's route to the host).
      // The default bridge gw (172.17.0.1) is wrong for containers on hub-hs-net.
      let hsNetGw = gw; // fallback to default bridge gw
      try {
        const hsGwOut = execSync(
          `docker network inspect hub-hs-net --format '{{range .IPAM.Config}}{{.Gateway}}\n{{end}}'`,
          { encoding: 'utf-8', timeout: 5_000 }
        )
          .trim()
          .split('\n')
      ...
      // 1:1 (Mill rate 1.0). A hub HS is a single-operator stack (apex +
      ...
      try {
        const connectorBridgeIp = execSync(
          `docker inspect ${HS_CONNECTOR_NAME} --format '{{(index .NetworkSettings.Networks "hub-hs-net").IPAddress}}'`,
          { encoding: 'utf-8', timeout: 5_000 }
        ).trim();
        if (
          connectorBridgeIp &&
      ...
      // hub-hs-town starts as part of the compose stack but isn't
      ...
      // Docker internal hostname 'hub-hs-town' resolves from the connector
      ...
      // the `hub-hs-net` bridge, BTP on port 3000 (NOT mapped to host).
      // Run the relay on the SAME bridge network so `connector:3000` resolves.
      console.log(
        '[49-5] Starting town relay (hub-hs-net, CONNECTOR_URL=ws://connector:3000)...'
      );
      const townImage = loadImageFromManifest('town');
      try {
        try {
          execSync(`docker rm -f hub-hs-town`, {
            stdio: 'pipe',
            timeout: 15_000,
          });
        } catch {
        ...
        // Use hub-hs-net gateway (same network as connector) so the relay can reach the host's Anvil
        ...
        const townState = execSync(
          `docker inspect hub-hs-town --format '{{.State.Status}}'`,
          { encoding: 'utf-8', timeout: 5_000 }
        ).trim();
        if (townState !== 'running') {
          const logs = execSync(
            'docker logs --tail 20 hub-hs-town 2>&1',
            { encoding: 'utf-8', timeout: 5_000 }
          );
          console.warn(
          ...
          console.log(
            '[49-5] Town relay started on hub-hs-net (CONNECTOR_URL=ws://connector:3000)'
          );
        }
      } catch (e) {
        console.warn(
      ...
      if (aDestination !== 'g.townhouse') {
        ...
        // Both are on hub-hs-net bridge so Docker DNS resolves 'connector'.
        ...
        let relayBtpUrl = 'ws://hub-hs-town:3000/btp';
        try {
          const relayIp = execSync(
            `docker inspect hub-hs-town --format '{{(index .NetworkSettings.Networks "hub-hs-net").IPAddress}}'`,
            { encoding: 'utf-8', timeout: 5_000 }
          ).trim();
      ...
      // ── Start DVM as Docker container (unauthenticated Turbo, hub-hs-net) ──
      console.log(
      ...
      // DVM container is running in standalone HTTP mode on hub-hs-net.
      ...
      // (hub-hs-mill:3000, on hub-hs-net) and registers it as a child
      ...
      try {
        await adminClientA.registerPeer({
          ...
          authToken: '',
          // transport:'direct' — dial Mill DIRECTLY on hub-hs-net, NOT through
    ...
    afterAll(async () => {
      ...
      try {
        ...
        cleanupAll();
        try {
          const orphans = execSync(`docker ps -aq --filter "name=hub-"`, {
            encoding: 'utf-8',
            timeout: 10_000,
          }).trim();
          if (orphans)
