...
// Contains: image-manifest.json + compose/hub-{hs,direct,dev}.yml (pre-rendered).
...
describe('loadComposeTemplate', () => {
  it('returns dev template verbatim', () => {
    ...
    expect(yaml).toContain('hub-dev-net');
...
describe('materializeComposeTemplate', () => {
  ...
  it('writes compose/hub-hs.yml AND image-manifest.json to tmpHome', () => {
    const { composePath, manifestPath } = materializeComposeTemplate('hs', {
      distDir: FIXTURE_DIR,
      hubHome: tmpHome,
    });
    expect(composePath).toBe(join(tmpHome, 'compose', 'hub-hs.yml'));
  ...
  it('is idempotent — second call overwrites first, mode stays 0o600, content matches, no errors', () => {
    const opts = { distDir: FIXTURE_DIR, hubHome: tmpHome };
  ...
  it('throws ComposeLoaderError for hs profile when manifest is absent', () => {
    ...
    try {
      ...
      copyFileSync(
        join(FIXTURE_DIR, 'compose', 'hub-hs.yml'),
        join(noManifestDir, 'compose', 'hub-hs.yml')
      );
      expect(() =>
        materializeComposeTemplate('hs', {
  ...
  it('does NOT throw for dev profile when manifest is absent', () => {
    ...
    try {
      mkdirSync(join(noManifestDir, 'compose'), { recursive: true });
      copyFileSync(
        join(FIXTURE_DIR, 'compose', 'hub-dev.yml'),
        join(noManifestDir, 'compose', 'hub-dev.yml')
      );
...
describe("'direct' compose profile (Phase 2 direct-apex)", () => {
  ...
  it('loadComposeTemplate accepts the direct profile and returns its YAML', () => {
    ...
    expect(yaml).toContain('hub-direct-net');
    expect(yaml).toContain('hub-direct-connector');
    ...
    expect(yaml).not.toContain('hub-hs-anon');
  });
  ...
  it('materializeComposeTemplate writes compose/hub-direct.yml', () => {
    const { composePath, manifestPath } = materializeComposeTemplate('direct', {
      distDir: FIXTURE_DIR,
      hubHome: tmpHome,
    });
    expect(composePath).toBe(join(tmpHome, 'compose', 'hub-direct.yml'));
    expect(() => statSync(composePath)).not.toThrow();
    expect(() => statSync(manifestPath)).not.toThrow();
    const mode = statSync(composePath).mode & 0o777;
    expect(mode).toBe(0o600);
  ...
  it('requires a manifest just like HS (throws when absent)', () => {
    ...
    try {
      mkdirSync(join(noManifestDir, 'compose'), { recursive: true });
      copyFileSync(
        join(FIXTURE_DIR, 'compose', 'hub-direct.yml'),
        join(noManifestDir, 'compose', 'hub-direct.yml')
      );
      expect(() =>
