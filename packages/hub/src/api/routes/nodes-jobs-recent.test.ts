...
beforeEach(async () => {
  ...
  connectorAdmin.setPeers([
    { id: 'hub-dev-dvm-01', ilpAddresses: ['g.test.dvm-01'] },
  ]);
...
describe('GET /nodes/:nodeId/jobs/recent', () => {
  it('happy path returns count, volume, byKind, byStatus', async () => {
    const res = await app.inject({
      method: 'GET',
      url: '/nodes/hub-dev-dvm-01/jobs/recent',
    });
  ...
  it('type !== dvm returns 404 with jobs_only_for_dvm', async () => {
    // Add a mill instance
    connectorAdmin.setPeers([
      { id: 'hub-dev-mill-01', ilpAddresses: ['g.test.mill-01'] },
    ]);
    const orchWithMill = {
      ...orchestrator,
      status: async () => [
        { name: 'hub-dev-mill-01', type: 'mill', state: 'running' },
      ],
    };
    ...
    const res = await app2.inject({
      method: 'GET',
      url: '/nodes/hub-dev-mill-01/jobs/recent',
    });
  ...
  it('windowSec=abc returns 400', async () => {
    const res = await app.inject({
      method: 'GET',
      url: '/nodes/hub-dev-dvm-01/jobs/recent?windowSec=abc',
    });
  ...
  it('windowSec=0 returns 400 (below minimum)', async () => {
    const res = await app.inject({
      method: 'GET',
      url: '/nodes/hub-dev-dvm-01/jobs/recent?windowSec=0',
    });
  ...
  it('windowSec=3601 returns 400 (above maximum)', async () => {
    const res = await app.inject({
      method: 'GET',
      url: '/nodes/hub-dev-dvm-01/jobs/recent?windowSec=3601',
    });
  ...
  it('windowSec=301 returns 400 (above DVM counter window)', async () => {
    ...
    const res = await app.inject({
      method: 'GET',
      url: '/nodes/hub-dev-dvm-01/jobs/recent?windowSec=301',
    });
    expect(res.statusCode).toBe(400);
  });
