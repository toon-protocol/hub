 ...
 * environment variable (set in hub-hs.yml compose env). The POST handler
 ...
 * `hub chains add --chain-type solana`) — the network presets carry no
...
function buildMillSwapPairConfig(config: HubConfig): object {
  ...
  // toChain is fixed at 'solana:devnet' for the v0.1 pilot — HubConfig
 ...
 * `hub hs up`). Returns undefined if the file is absent or unparseable —
      ...
      try {
        ...
        if (type === 'mill' && millRelays.length === 0) {
          return reply.status(400).send({
            step: 'preflight',
            err: 'No relay URLs for Mill. Pass `--relays wss://relay1,wss://relay2` to `hub node add mill`, set nodes.mill.relays in config.yaml, or export MILL_RELAYS before `hub hs up`. See packages/hub/README.md.',
          });
        }
        ...
        try {
          await deps.connectorAdmin.registerPeer({
            ...
            // `TOON_PARENT_PEER_ID` in hub-hs.yml (which MUST equal the
