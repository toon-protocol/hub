...
beforeEach(async () => {
  ...
  homeDir = await fs.mkdtemp(join(tmpdir(), 'hub-lc-test-'));
...
describe('POST /api/nodes success path', () => {
  ...
  it('provisions mill node — writes mill.config.json with default shape', async () => {
    ...
    expect(body.healthCheckUrl).toBe('http://hub-hs-mill:3200/health');
