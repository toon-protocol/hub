...
/**
 * Build a HubConfig with selected nodes enabled.
 */
...
describe('DockerOrchestrator', () => {
  ...
  // ── T-007: hub up --town --mill starts connector + Town + Mill ──
  ...
  describe('up() — connector health gating (T-008)', () => {
    it('waits for connector health before starting node containers', async () => {
      ...
      expect(startOrder.indexOf('hub-connector')).toBeLessThan(
        startOrder.indexOf('hub-town')
      );
    });
  ...
  describe('down() — graceful shutdown (T-009)', () => {
    it('stops node containers before connector', async () => {
      ...
      mockDocker.docker.listContainers.mockResolvedValue([
        { Names: ['/hub-connector'], State: 'running' },
        { Names: ['/hub-town'], State: 'running' },
        { Names: ['/hub-mill'], State: 'running' },
      ]);
      ...
      const connectorIdx = stopOrder.indexOf('hub-connector');
      const townIdx = stopOrder.indexOf('hub-town');
      const millIdx = stopOrder.indexOf('hub-mill');
    ...
    it('removes hub-net network after stopping all containers', async () => {
      mockDocker.docker.listContainers.mockResolvedValue([]);
      mockDocker.docker.listNetworks.mockResolvedValue([
        { Name: 'hub-net', Id: 'net-123' },
      ]);
  ...
  describe('down() — edge cases', () => {
    it('handles already-stopped containers gracefully (no throw)', async () => {
      mockDocker.docker.listContainers.mockResolvedValue([
        { Names: ['/hub-connector'], State: 'exited' },
      ]);
  ...
  describe('healthCheck() — configurable polling (T-015)', () => {
    it('respects custom polling interval', async () => {
      ...
      const result = await orchestrator.healthCheck('hub-connector', {
        interval: 10,
        timeout: 10000,
      });
    ...
    it('times out when container never becomes healthy', async () => {
      ...
      await expect(
        orchestrator.healthCheck('hub-connector', {
          interval: 10,
          timeout: 50,
        })
  ...
  describe('ensureNetwork() — Docker network (AC #1)', () => {
    it('creates hub-net bridge network if it does not exist', async () => {
      mockDocker.docker.listNetworks.mockResolvedValue([]);
    ...
    it('skips network creation if hub-net already exists', async () => {
      mockDocker.docker.listNetworks.mockResolvedValue([
        { Name: 'hub-net', Id: 'existing-net' },
      ]);
  ...
  describe('container environment variables (AC #1)', () => {
    ...
    it('passes town-specific env vars from config', async () => {
      ...
      expect(mockDocker.docker.createContainer).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          Env: expect.arrayContaining([
            'FEE_PER_EVENT=1000',
            'CONNECTOR_URL=ws://hub-connector:3000',
          ]),
        })
      );
    });
    ...
    it('ensureRelaySidecar() creates the relay sidecar with HS-correct env/network/volume', async () => {
      ...
      // path used hub-net / hub-town and never resolved), and a
      // named keypair volume for a stable .anyone address.
      expect(mockDocker.docker.createContainer).toHaveBeenCalledWith(
        expect.objectContaining({
          name: 'hub-hs-ator-sidecar-relay',
          Image: 'toon:hub-ator-sidecar',
          Env: expect.arrayContaining([
            'HS_TARGET_HOST=hub-hs-town',
            'HS_TARGET_PORT=7100',
            'HS_PORT=7100',
            'SOCKS_PORT=9051',
          ]),
          HostConfig: expect.objectContaining({
            NetworkMode: 'hub-hs-net',
            Binds: expect.arrayContaining([
              'hub-hs-relay-anon:/var/lib/anon/hs:rw',
            ]),
          }),
        })
      );
    ...
    it('ensureRelaySidecar() is idempotent — skips creation when already running', async () => {
      ...
      await orchestrator.ensureRelaySidecar();
      expect(mockDocker.docker.createContainer).not.toHaveBeenCalledWith(
        expect.objectContaining({ name: 'hub-hs-ator-sidecar-relay' })
      );
    });
    ...
    it('down() stops the relay ator sidecar alongside other hub containers', async () => {
      const stopped: string[] = [];

      mockDocker.docker.listContainers.mockResolvedValue([
        { Names: ['/hub-connector'], State: 'running' },
        { Names: ['/hub-town'], State: 'running' },
        { Names: ['/hub-ator-sidecar-relay'], State: 'running' },
      ]);
      mockDocker.docker.getContainer.mockImplementation((name: string) => ({
        start: vi.fn().mockResolvedValue(undefined),
        stop: vi.fn().mockImplementation(async () => {
      ...
      expect(stopped).toContain('hub-ator-sidecar-relay');
      // sidecar is stopped before the connector (parallel "node" group)
      expect(stopped.indexOf('hub-ator-sidecar-relay')).toBeLessThan(
        stopped.indexOf('hub-connector')
      );
    });
  ...
  describe('addNode() (T-018)', () => {
    it('starts new node and regenerates connector config', async () => {
      ...
      expect(callOrder).toContain('create:hub-mill');
      expect(callOrder).toContain('create:hub-connector');
    });
  });
  ...
  describe('removeNode() (T-018)', () => {
    it('stops node and regenerates connector config without it', async () => {
      ...
      expect(stoppedContainers).toContain('hub-mill');
      ...
      const createCall = mockDocker.docker.createContainer.mock.calls.find(
        (call: any[]) => call[0].name === 'hub-connector'
      );
  ...
  describe('up() — full startup sequence (AC #1, #3, #4, #6, #7)', () => {
    it('executes startup in correct order: network -> pull -> connector -> nodes', async () => {
      ...
      expect(callOrder.indexOf('pull')).toBeLessThan(
        callOrder.indexOf('create:hub-connector')
      );
      // Connector created/started before nodes
      expect(callOrder.indexOf('start:hub-connector')).toBeLessThan(
        callOrder.indexOf('start:hub-town')
      );
      expect(callOrder.indexOf('start:hub-connector')).toBeLessThan(
        callOrder.indexOf('start:hub-mill')
      );
    });
  });
    ...
    it('swallows "no such service" stderr — real-world docker compose rm stderr string', async () => {
      ...
      try {
        ...
        const noSuchContainerStderr =
          'no containers to remove\nservice "mill" is not running container "hub-hs-mill"';
        const exec: Exec46 = () => {
