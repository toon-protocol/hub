...
describe('serviceFromContainerName', () => {
  it('maps single-instance container names', () => {
    expect(serviceFromContainerName('hub-town')).toBe('town');
    expect(serviceFromContainerName('hub-mill')).toBe('mill');
    expect(serviceFromContainerName('hub-dvm')).toBe('dvm');
    expect(serviceFromContainerName('hub-connector')).toBe('connector');
  });
  ...
  it('maps preset multi-instance container names', () => {
    expect(serviceFromContainerName('hub-dev-town-01')).toBe('town');
    expect(serviceFromContainerName('hub-dev-mill-02')).toBe('mill');
    expect(serviceFromContainerName('hub-dev-dvm-01')).toBe('dvm');
  });

  it('returns null for non-hub containers', () => {
    expect(serviceFromContainerName('redis')).toBeNull();
    expect(serviceFromContainerName('hub-foo')).toBeNull();
  });
});
