/**
 * Docker Orchestration Engine for Hub (Story 21.2).
 *
...
const RELAY_ATOR_SIDECAR_IMAGE = 'toon:hub-ator-sidecar';
 ...
 * ACTUAL network + town container names (compose/hub-hs.yml: network
 * `hub-hs-net`, town container `hub-hs-town`) — NOT the dev-profile
 ...
 */
const RELAY_HS_NETWORK = 'hub-hs-net';
...
/**
 * DockerOrchestrator manages the lifecycle of Hub containers.
 *
...
export class DockerOrchestrator extends EventEmitter {
  ...
  private async upDev(profiles: NodeType[]): Promise<void> {
    ...
    await this.waitForHealth('hub-connector');
  ...
  private async upHs(profiles: NodeType[]): Promise<void> {
    ...
    // so the operator can retry without a manual `hub hs down`.
    try {
  ...
  private surfaceComposeFailure(stderr: string): void {
    ...
    // (e.g., `failed to start "hub-api"`).
    // Pattern 3 was hardcoded to `hub-hs-` which would miss
  ...
  private async downHs(): Promise<void> {
    ...
    // (createContainer) and holds the hub-hs-net network, so `compose
    ...
    // NO -v flag — preserves the hub-hs-anon volume so the .anyone
    ...
    try {
      ...
      if (e.code === 'ENOENT') {
        ...
        // such service) as success so `hub hs down` is idempotent.
        if (
   ...
   *
   * @param containerName - Full container name (e.g. 'hub-town')
  ...
  /**
   * Return status for all hub containers.
   *
   * Discovers both single-instance (hub-<type>) and multi-instance
   * (hub-<prefix>-<type>-<n>) containers. Multi-instance containers
  ...
  > {
    ...
    // Collect all containers that belong to this hub instance
    const matching: {
      containerName: string;
  ...
  async pullImages(profiles: NodeType[]): Promise<void> {
    ...
    // by docker/hub-ator-sidecar — pull may 404, which is fine; the
    // operator must have built it before `hub up` (see README).
  ...
  /**
   * Create the hub-net bridge network if it doesn't exist.
   */
   ...
   * docker/hub-ator-sidecar/Dockerfile). The town container picks up
  ...
  /**
   * Remove the hub-net network if it exists.
   */
  ...
  private async buildNodeEnv(type: NodeType): Promise<string[]> {
    ...
    switch (type) {
      ...
      case 'dvm': {
        ...
        // throws on first upload with a `hub credits buy` CTA.
        //
