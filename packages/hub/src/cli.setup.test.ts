/**
 * Tests for `hub setup` CLI command (AC-1).
 */
...
function makeTempDir(): string {
  const dir = join(
    tmpdir(),
    `hub-setup-test-${randomBytes(8).toString('hex')}`
  );
...
describe('CLI setup command (AC-1)', () => {
  ...
  it('short-circuits when both config.yaml and wallet.enc already exist', async () => {
    ...
    try {
      ...
      expect(output).toContain('hub up');
    } finally {
...
`
      ...
      // a "Run `hub init` first" message when the wallet is absent.
      ...
      expect(errorOutput).toContain('hub init');
      expect(process.exitCode).toBe(1);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
