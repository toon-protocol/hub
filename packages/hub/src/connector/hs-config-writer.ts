/**
 * Apex connector config writer for `hub hs up` / `hub up --transport
...
import type { ChainProviderEntry, HubConfig } from '../config/schema.js';
 ...
 */
const HS_DVM_HANDLER_URL = 'http://hub-hs-dvm:3300';
const DIRECT_DVM_HANDLER_URL = 'http://hub-direct-dvm:3300';
 ...
 *
 * Precedence:
 *   1. explicit config.chainProviders (operator ran `hub chains add`)
...
/**
 * Write (or reuse) `~/.hub/connector.yaml` with HS-specific overrides.
 *
 * @param configDir - The hub home directory (e.g. `~/.hub/`).
 * @param config - Loaded `HubConfig` (provides adminPort, ilpAddress, etc.).
...
/**
 * Write (or reuse) `~/.hub/connector.yaml` for DIRECT (no-HS) mode.
 *
...
/**
 * Detect whether `~/.hub/connector.yaml` already describes a HIDDEN-SERVICE
 * apex (`anon.enabled: true`) — the same marker the HS writer's idempotency
 * check keys on. Used by the back-compat guard so the direct-default `hub
