/**
 * Connector Config Generator for Hub (Story 21.3).
 *
 * Generates runtime configuration for the standalone ILP connector
 * based on the Hub config and currently active nodes.
 */
...
/** Default ILP address for the Hub connector */
 ...
 * ILP connector based on Hub config and active node list.
 *
...
export class ConnectorConfigGenerator {
  ...
  toEnvVars(runtimeConfig: ConnectorRuntimeConfig): Record<string, string> {
    ...
    // address out-of-band (e.g., `hub status` reading the hostname
   ...
   *
   * The shape mirrors `docker/configs/hub-dev-connector.yaml` (the
  ...
  toYaml(runtimeConfig: ConnectorRuntimeConfig): string {
    ...
    // /admin/earnings.json returns 503 and Hub's earnings data plane
   ...
   * BTP URLs use Docker DNS: btp+ws://hub-{type}:3000 // nosemgrep: javascript.lang.security.detect-insecure-websocket.detect-insecure-websocket
   */
  ...
  /**
   * Generate transport config from Hub config.
