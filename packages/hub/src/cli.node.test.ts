...
describe('CLI node subcommand', () => {
  ...
  it('node add with no type → POSTs {type: "town"} (AC #3)', async () => {
    const overrides = makeNodeOverrides(201, {
      ...
      healthCheckUrl: 'http://hub-hs-town:3100/health',
    });
  ...
  it('node add mill → POSTs {type: "mill"} and prints four-stage progress', async () => {
    const overrides = makeNodeOverrides(201, {
      ...
      healthCheckUrl: 'http://hub-hs-mill:3200/health',
    });
    await main(
      ['node', 'add', 'mill'],
      undefined,
  ...
  it('node add dvm --turbo-token → POSTs turboToken in the body', async () => {
    const overrides = makeNodeOverrides(201, {
      ...
      healthCheckUrl: 'http://hub-hs-dvm:3400/health',
    });
    await main(
  ...
  it('node add against unreachable API → exit 1, stderr includes "hub hs up"', async () => {
    ...
    expect(process.exitCode).toBe(1);
    expect(stderrText()).toContain('hub hs up');
  });
  ...
  it('node add --help → stdout documents the mill --relays example (AC #7)', async () => {
    ...
    expect(out).toContain('hub node add mill --relays');
  ...
  it('node list empty → prints empty-state hint', async () => {
    ...
    expect(out).toContain('hub node add town');
  });
