...
describe('handleLogs', () => {
  it('resolves hub- prefixed container names verbatim', async () => {
    const listContainersMock = vi
      .fn()
      .mockResolvedValue([{ Names: ['/hub-connector'] }]);
    ...
    await handleLogs(mockDocker, 'hub-connector', {
      lines: 50,
      json: false,
      jsonCompact: false,
      docker: mockDocker,
  ...
  it('resolves bare service tag to hub-<tag> when unambiguous', async () => {
    const listMock = vi
      .fn()
      .mockResolvedValue([{ Names: ['/hub-connector'] }]);
