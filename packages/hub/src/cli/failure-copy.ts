...
const FAILURE_COPY: Readonly<Record<string, FailureCopyEntry>> = {
  'anon-timeout': {
    ...
    nextStep: 'Re-run with DEBUG=hub:* for verbose anon logs.',
  },
  'anon-disabled': {
    headline: 'Connector is anon-disabled.',
    explanation: 'The connector config has anon.enabled: false.',
    nextStep: 'Edit ~/.hub/connector.yaml and set anon.enabled: true.',
  },
  'image-pull-failure': {
    headline: 'Image pull failed.',
    explanation: 'Docker could not pull the required hub images.',
  ...
  'missing-docker-sock': {
    ...
    nextStep: 'Start Docker and re-run `hub hs up`.',
  },
  generic: {
    headline: 'Apex boot failed.',
    explanation: '',
    nextStep: 'Run with DEBUG=hub:* for verbose logs.',
  },
};
...
export function renderFailure(error: unknown): { exitCode: number } {
  ...
  if (!entry) {
    ...
    process.stderr.write(
      `  ${arrow} Run with DEBUG=hub:* for verbose logs.\n`
    );
    return { exitCode: 1 };
  }
