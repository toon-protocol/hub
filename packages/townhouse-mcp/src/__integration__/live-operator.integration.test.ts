 ...
 * `townhouse init`→`up`→`add_node`→`earnings`→`withdraw(dryRun)` against local
 * Docker chains, driven entirely through the MCP dispatch surface.
 *
 * Skipped unless RUN_LIVE_OPERATOR_E2E=1. Requires the townhouse CLI on PATH (or
...
describe.skipIf(!RUN)('live operator E2E', () => {
  ...
  it('reports status (api or cli) without throwing', async () => {
    const res = await dispatchTool(ctx, 'townhouse_status', {});
    expect(res.content[0]!.text.length).toBeGreaterThan(0);
  });

  it('lists nodes once the apex is up', async () => {
    const res = await dispatchTool(ctx, 'townhouse_list_nodes', {});
  ...
  it('estimates a withdraw via dryRun', async () => {
    const res = await dispatchTool(ctx, 'townhouse_withdraw', {
      nodeType: 'town',
      chainFamily: 'evm',
      token: 'native',
