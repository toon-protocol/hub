 ...
 * the long-running `townhouse up` / `hs up` bootstrap. We reuse client-mcp's
 * detached-spawn + append-log idiom (detached + `unref()` + `openSync(log,'a')`)
 * for a different reason than client-mcp: `townhouse up` is a bootstrap command
 ...
 * regardless of MCP-process lifetime. See docs/townhouse-mcp-design.md §0/§5.
 */
...
/**
 * Spawn `townhouse up` (or `hs up`) DETACHED, NDJSON progress appended to
...
export function spawnUpDetached(
  ...
  if (child.pid === undefined) {
    throw new Error('Failed to spawn `townhouse up` (no pid)');
  }
  return child.pid;
}
