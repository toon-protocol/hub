...
describe('resolveConfig', () => {
  it('applies defaults for an empty env', () => {
    ...
    expect(cfg.configDir).toBe(join(homedir(), '.townhouse'));
    expect(cfg.townhouseBin).toBe('townhouse');
  ...
  it('reads each override from the env', () => {
    const cfg = resolveConfig({
      ...
      TOWNHOUSE_BIN: '/usr/local/bin/townhouse',
      TOWNHOUSE_AUTOUP: '0',
    ...
    expect(cfg.townhouseBin).toBe('/usr/local/bin/townhouse');
