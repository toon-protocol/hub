/**
 * Drives the `hub` CLI for lifecycle + config + money mutations that the
 ...
 * hub layer — docs/hub-mcp-design.md §3).
 */
import { spawn } from 'node:child_process';
import type { ResolvedConfig } from './config.js';

/** Error thrown when a `hub` invocation exits non-zero. */
...
/**
 * Thrown when the `hub` binary itself can't be spawned (ENOENT) — i.e. the
 ...
 * `spawn hub ENOENT`. CLI-backed tools (`health`, `channels`, `chains
...
export class CliNotFoundError extends Error {
  constructor(readonly bin: string) {
    super(
      `hub CLI not found (tried to spawn "${bin}"). Set TOWNHOUSE_BIN to ` +
        `the CLI entry point (e.g. node_modules/@toon-protocol/hub/dist/cli.js ` +
        `or packages/hub/dist/cli.js in a checkout), or install ` +
        `@toon-protocol/hub so the \`hub\` command is on PATH. ` +
...
export class CliDriver {
   ...
   * `hub health` exits 1 when any probe is unhealthy yet still prints the
...
function defaultExec(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('error', (e: NodeJS.ErrnoException) => {
      ...
      // `spawn hub ENOENT` into an actionable "set TOWNHOUSE_BIN" error.
