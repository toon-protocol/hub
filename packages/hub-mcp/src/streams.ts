 ...
 * back to the (reliable) `hub` CLI JSON path. The WS factory defaults to
 ...
 * `LogEvent` (docker/log-tail), which is not part of the hub public
...
export class StreamsUnavailableError extends Error {
  constructor(
    ...
    super(`hub stream not available at ${url}`);
  ...
  if (!Ctor) {
    throw new Error(
      'global WebSocket unavailable in this runtime (need Node >=21); ' +
        'metrics will fall back to the hub CLI'
    );
  }
  return new Ctor(url);
}
