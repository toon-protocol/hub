...
describe('CliDriver.runJsonLenient', () => {
  it('returns the JSON payload even when the command exits non-zero', async () => {
    // `hub health` exits 1 on an unhealthy probe but still reports.
    const d = new CliDriver(
      cfg,
...
describe('defaultExec (real spawn)', () => {
  it('throws CliNotFoundError when the hub binary is missing (ENOENT)', async () => {
    // No injected exec → real defaultExec spawns this bogus path, which ENOENTs.
    const d = new CliDriver({
      ...cfg,
      hubBin: '/nonexistent/hub-please-do-not-exist',
    });
